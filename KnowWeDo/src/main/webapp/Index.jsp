    ...
    <body onload="initialize()" >
        ...
        <script type="text/javascript">
            ...
            <%
                ...
                boolean[] linkAddress = new boolean[97];
                for (Address address : linkFilterAddress) {
                    linkAddress[address.getZipCode() / 100] = true;
                }
            %>
            ...
            <%
                for (int i = 0; i < linkAddress.length; i++) {
                    if (linkAddress[i]) {
            %>
                mmmap.showObject('<%=i%>', "IG", true, null, null, "FF8000", "FF8000", 0.7, 0.5);
            <%      } else {
            %>
                mmmap.showObject('<%=i%>', "IG", true, null, null, "4d4d4d", "4d4d4d", 0.7, 0.5);
            <%      }
                }
            %>
            ...
            %>
                ...
                if (Math.trunc(address.geocode / 10000) == <%=((Address) linkFilterAddress.toArray()[i]).getZipCode() / 100%>) {
                    mmmap = new MMMap(mmMapDiv, 13.7648787, 100.5380879, 3, "normal");
                    mmmap.showObject(Math.trunc(address.geocode / 10000) + '__', "IG", true, null, null, "4d4d4d", "4d4d4d", 0.7, 0.5);
                    mmmap.setSize(700, 700);
                    mmmap.rePaint();
                    mmmap.hideZoomBar();
                    mmmap.hideVerticalZoomBar();
...
>>>>>>> 17294be3ada68f87fcbdc2c2cdbc5ee96eb67fb1
